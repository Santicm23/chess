...
def rect_circle(color,x,y,lo,la):
    m = max(lo,la)
...
class Board:#*#*molde del tablero
                ...
                if not self.position[i][j] == 0:
                    if check[0]:
                        if piece.type == 6 or [i,j] == check[1].pos:
                            self.reset_sq([i,j],GREY,LGREY)
                            screen.blit (self.position[i][j].image,(j*80,i*80))
    ...
    def reset_lm(self):#*#resetear las jugadas de todas la piezas presentes en el tablero
        for wp in Wpieces:
            wp.rsrch_lm(self)
        for bp in Bpieces:
            bp.rsrch_lm(self)
    ...
    def reset_zone(self,pos):#*#repintar una zona del tablero de 9x9 para el movimiento correctode las piezas
        (i,j) = pos
        if i-int(i)>=0.5:
            i = int(i+1)
        elif i-int(i)<-0.5:
        ...
        if not self.position[j][i] == 0:
            screen.blit (self.position[j][i].image,(i*80,j*80))
        if not i == 7:
            board.reset_sq((j,i+1),GREY,LGREY)
            if not self.position[j][i+1] == 0:
                screen.blit (self.position[j][i+1].image,((i+1)*80,j*80))
        if not j == 7:
            board.reset_sq((j+1,i),GREY,LGREY)
            if not self.position[j+1][i] == 0:
                screen.blit (self.position[j+1][i].image,(i*80,(j+1)*80))
        if not i == 0:
            board.reset_sq((j,i-1),GREY,LGREY)
            if not self.position[j][i-1] == 0:
                screen.blit (self.position[j][i-1].image,((i-1)*80,j*80))
        ...
        if not i == 7 and not j == 7:
            ...
            if not self.position[j+1][i+1] == 0:
                screen.blit (self.position[j+1][i+1].image,((i+1)*80,(j+1)*80))
                    ...
                    for k2 in range (2):
                        ...
                        if new_pos[0]+a == self.pos[0] and new_pos[1]+b == self.pos[1]:
                            if not  new_board.get_piece(columnes[new_pos[1]],lines[new_pos[0]]) == 0 and new_board.get_piece(columnes[new_pos[1]],lines[new_pos[0]]).color == WHITE:
                                ...
                                return True
def lmc_bishop(self,new_pos,t,new_board):
                ...
                for d in range (-1,2,2):
                    ...
                    if self.pos[0]+q*c>-1 and self.pos[0]+q*c<8 and self.pos[1]+q*d>-1 and self.pos[1]+q*d<8 and not new_board.position[self.pos[0]+q*c][self.pos[1]+q*d] == 0:
                        if new_pos[0]==self.pos[0]+q*c and new_pos[1]==self.pos[1]+q*d and new_board.get_piece(columnes[new_pos[1]],lines[new_pos[0]]).color == WHITE:
                            return True
def lmc_rook(self,new_pos,t,new_board):
                ...
                for j in range (2):
                    ...
                    if self.pos[0]+q*a>-1 and self.pos[0]+q*a<8 and self.pos[1]+q*b>-1 and self.pos[1]+q*b<8 and not new_board.position[self.pos[0]+q*a][self.pos[1]+q*b] == 0:
                        if new_pos[0]==self.pos[0]+q*a and new_pos[1]==self.pos[1]+q*b and  new_board.get_piece(columnes[new_pos[1]],lines[new_pos[0]]).color == WHITE:
                            return True
def lmc_queen(self,new_pos,t,new_board):
            ...
            for c in range (-1,2):
                for d in range (-1,2):
                    ...
                    if self.pos[0]+q*c>-1 and self.pos[0]+q*c<8 and self.pos[1]+q*d>-1 and self.pos[1]+q*d<8 and not new_board.position[self.pos[0]+q*c][self.pos[1]+q*d] == 0:
                        if new_pos[0]==self.pos[0]+q*c and new_pos[1]==self.pos[1]+q*d and new_board.get_piece(columnes[new_pos[1]],lines[new_pos[0]]).color == WHITE:
                            return True
def lmc_king(self,new_pos,t,new_board):
...
#*#*codigo de piezas, tablero e imagenes
...
brd = 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1'
...
while running:#*#*bucle principal
    ...
    (m,n) = pos
    cnt = cnt + 1
    if not piece_raised == 0 and not promoting[0] and not game_over():
        m = int(m/80)
        n = int(n/80)
        if cnt % 6:
            if not move:
            ...
            if len(mouse_sq) == 2 and not mouse_sq == (n,m):
                (a,b) = mouse_sq
                board.reset_sq(mouse_sq,GREY,LGREY)
                if mselect == mp:
                    screen.blit(mp,(b*80+30,a*80+30))
                elif mselect == cp:
                    screen.blit(cp,(b*80,a*80))
                    if not board.position[a][b] == 0:
                        screen.blit (board.position[a][b].image,(b*80,a*80))
        ...
        if event.type == MOUSEBUTTONDOWN:#*#click
            ...
            if piece_raised == 0 and not promoting[0] and not game_over():#*levantar pieza (1° click)
                ...
                if not board.position[j][i] == 0:
                    ...
                    piece_raised = 0
                move = False
            elif promoting[0] and not game_over():#*solo si esta coronando un peon(3°click)
                (r,c) = promoting[1].pos
                (y,x) = promoting[3]
                pawn = promoting[1]
                if pawn.color == WHITE:
