...
chessfont = pygame.font.Font('chess_pepega/CASEFONT.TTF', 80)
...
class Board:
    ...
    def reset_sq(self,pos,c1,c2):#repinta una sola casilla del tablero
        ...
        if xp % 2 and yp % 2 or (xp-1) % 2 and (yp-1) % 2:
            ...
            if xp == 7:
                ltr=font.render(columnes[y],1,c1)
                screen.blit(ltr,(71+yp*80, 67+xp*80))
            elif yp == 0:
                nbr=font.render(lines[x],1,c1)
                screen.blit(nbr,(2+yp*80, xp*80+2))
        else:
            pygame.draw.rect(screen, c1, (yp*80, xp*80, 80, 80))
            if xp == 7:
                ltr=font.render(columnes[y],1,c2)
                screen.blit(ltr,(71+yp*80, 67+xp*80))
            if yp == 0:
                nbr=font.render(lines[x],1,c2)
                screen.blit(nbr,(2+yp*80, xp*80+2))
