...
class Board:#*#*molde del tablero
    ...
    def reset(self,o):#*#reiniciar la parte grafica
        for j in range (8):
            for i in range (8):
                self.reset_sq((j,i),GREY,LGREY)
                i1 = 7-i
                j1 = 7-j
                if not self.position[j][i] == 0 and o == 0:
                    screen.blit (self.position[j][i].image,(i*80,j*80))
                elif not self.position[j1][i1] == 0 and o == 1 :
    ...
    def promote(self,new_pos,color,o):#*#mostrar recuadro de pizas para coronar
        (c,r) = new_pos
        s = 1
        z = 0
        if o == 1:
                ...
                if not old_pos == piece_raised.pos:
                    ...
                    if piece_raised.type == 1:
                        if piece_raised.pos[0] == 0:
                            ...
                            board.reset(nt)
                            board.promote(piece_raised.pos,BLACK,orientation)
                        elif piece_raised.pos[0] == 7:
                            promoting = [True,piece_raised,pcd,old_pos]
                            board.reset(nt)
                            board.promote(piece_raised.pos,WHITE,orientation)
                        else:
                            board.reset_lm()
                            board.reset(orientation)
                    else: 
